...
@Repository
public interface SqlJob extends PagingAndSortingRepository<CarModelEntity, Long> {
    ...
    @Query(value =
            ...
            "where ((cue.kilometers_since_the_last_change_braek_pads>=cme.brake_pads) or " +
            "(cue.kilometers_since_the_last_change_oil>=cme.oil)or(cue.kilometers_since_the_last_change_timing_gear>=cme.timing_gear)) " +
            ...
            "from " +
            "car_user_entity cue join car_model_entity cme on cme.id=cue.car_model " +
            "where(cue.kilometers_since_the_last_change_timing_gear>=cme.timing_gear) ", nativeQuery = true)
    void job();
